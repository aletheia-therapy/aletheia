...
function TherapyContent() {
  ...
  // 輸入區元件（共用）
  const InputArea = () => (
    <div className={`${isMobile ? 'relative z-20 border-t border-white/10 bg-black/80 px-2 pt-2 pb-6' : 'p-4 border-t border-white/10 bg-white/5'}`}>
  ...
  // 訊息列表（共用）
  const MessageList = () => (
    <div className="max-w-3xl mx-auto px-2 py-4 space-y-3">
      {messages.length === 0 && (
  ...
  if (!isMobile) {
    return (
      <div className="h-screen bg-black flex flex-col overflow-hidden relative">
        ...
        {/* 頂部 */}
        <div className="relative z-30 flex justify-between items-center px-4 py-2 backdrop-blur-md bg-black/40 border-b border-white/5 flex-shrink-0">
          <button onClick={() => router.push('/')} className="bg-white/10 px-3 py-1 rounded-full border border-white/20 text-white/70 text-sm">← 首頁</button>
          <span className="text-white/40 text-sm">{messageCount}/{MESSAGE_LIMIT} · {intentNames[intent] || intent}</span>
          <div className="flex gap-2">
            <button onClick={handleViewMindMap} disabled={messages.length === 0} className="bg-white/10 px-3 py-1 rounded-full border border-white/20 text-white/70 text-sm disabled:opacity-40">🗺️ 地圖</button>
            <button onClick={handleClearChat} disabled={messages.length === 0} className="bg-white/10 px-3 py-1 rounded-full border border-white/20 text-white/70 text-sm disabled:opacity-40">🔄</button>
          </div>
        </div>
        ...
        {/* 輸入區 - 固定在底部 */}
        <div className="relative z-20 flex-shrink-0 backdrop-blur-md">
          <InputArea />
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-black relative">
      ...
      <div className="sticky top-0 z-30 flex justify-between items-center px-2 py-2 backdrop-blur-md bg-black/40 border-b border-white/5">
        <button onClick={() => router.push('/')} className="bg-white/10 px-3 py-1 rounded-full border border-white/20 text-white/70 text-xs">← 首頁</button>
        <span className="text-white/40 text-xs">{messageCount}/{MESSAGE_LIMIT} · {intentNames[intent] || intent}</span>
        <div className="flex gap-2">
          <button onClick={handleViewMindMap} disabled={messages.length === 0} className="bg-white/10 px-3 py-1 rounded-full border border-white/20 text-white/70 text-xs disabled:opacity-40">🗺️</button>
          <button onClick={handleClearChat} disabled={messages.length === 0} className="bg-white/10 px-3 py-1 rounded-full border border-white/20 text-white/70 text-xs disabled:opacity-40">🔄</button>
        </div>
      </div>

      {/* 錯誤訊息 */}
      {errorMessage && (
        <div className="mx-2 mt-2 z-40">
