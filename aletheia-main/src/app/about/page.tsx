    ...
    <div className="relative min-h-screen overflow-hidden bg-black flex flex-col">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center px-4 py-16">
        ...
        <div className="text-center mb-16 animate-fade-in">
          <h1 className="text-4xl font-light text-white mb-3 tracking-wider">關於 Aletheia 阿勒希雅</h1>
          <p className="text-white/50 text-base">Aletheia，希臘文意指「真理」——讓真實的自己被看見</p>
        </div>
        <div className="w-full max-w-3xl mb-10 animate-fade-in" style={{ animationDelay: '0.1s' }}>
          <div className="backdrop-blur-sm bg-white/5 rounded-2xl border border-white/10 p-8">
            ...
            <div className="space-y-4 text-white/70 leading-relaxed text-sm">
              ...
              <p><span className="text-purple-300">第一，因為親身走過。</span>我們自己也曾有過心理困擾的時刻——那種深夜找不到人說話、不知道怎麼開口、或是說了也怕被誤解的感受。我們知道那種孤獨是什麼感覺，所以想為更多人建一個出口。</p>
              <p><span className="text-purple-300">第二，因為相信 AI 可以更溫柔。</span>AI 不只是回答問題的工具。它可以不評判、不疲憊、不遺忘你說過的每一句話。如果用對了方式，它可以成為一個真正陪伴的存在——尤其在人與人之間越來越難以坦誠的時代。</p>
              <p><span className="text-purple-300">第三，因為心理資源應該更容易取得。</span>預約一個心理師，往往需要等待、需要費用、需要勇氣跨出第一步。我們希望阿勒希雅能成為那個「第一步」——讓更多人在準備好之前，先有一個地方可以說說話。</p>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-3xl mb-10 animate-fade-in" style={{ animationDelay: '0.4s' }}>
          <div className="backdrop-blur-sm bg-red-500/5 rounded-2xl border border-red-500/20 p-8">
            ...
            <p className="text-white/40 text-xs mb-6">如果你現在感到非常痛苦，或有傷害自己的念頭，請立即聯繫以下專線：</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {helplines.map((line) => (
                <a key={line.name} href={`tel:${line.number}`} className="flex items-center gap-4 p-4 bg-white/5 rounded-xl border border-white/10 hover:border-red-400/30 hover:bg-red-500/10 transition-all duration-200 group">
                  <div className="text-2xl font-bold text-red-300 group-hover:text-red-200 transition-colors min-w-[60px] text-center">{line.number}</div>
                  <div>
        ...
        <div className="w-full max-w-3xl mb-10 animate-fade-in" style={{ animationDelay: '0.5s' }}>
          <div className="backdrop-blur-sm bg-white/5 rounded-2xl border border-white/10 p-8 text-center">
            ...
            <p className="text-white/50 text-sm mb-6 leading-relaxed">如果你有任何建議、回饋，或想支持我們繼續走下去，歡迎來找我們說話。</p>
            <a href="https://www.facebook.com/profile.php?id=61584252840273" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-purple-500/20 border border-purple-500/30 text-purple-300 hover:bg-purple-500/30 transition-all duration-200 text-sm">
              <span>📘</span> 心一線上心理諮詢室 Facebook
            </a>
          </div>
        </div>
        <div className="animate-fade-in" style={{ animationDelay: '0.6s' }}>
          <Link href="/" className="inline-flex items-center gap-2 px-8 py-3 rounded-full backdrop-blur-sm bg-white/10 border border-white/20 text-white/70 hover:text-white hover:bg-white/20 transition-all duration-200 text-sm">
            開始與阿勒希雅對話 →
          </Link>
        </div>
      </div>
